...
function ChannelPlayer() {
  // Ref to access the comment input field
  const commentInputRef = useRef(null);
  // Ref to access the comment button
  const commentBtnRef = useRef(null);
  // When comment input is focused, make the button visible & change color
  const commentFocus = () => {
    commentBtnRef.current.classList.replace("bg-stone-300/60", "bg-blue-500");
    commentBtnRef.current.classList.replace("hidden", "block");
  };
  // When comment input loses focus, reset button color
  const commentBlur = () => {
    commentBtnRef.current.classList.replace("bg-blue-500", "bg-stone-300/60");
  };
  // Get video and channel info passed via React Router location state
