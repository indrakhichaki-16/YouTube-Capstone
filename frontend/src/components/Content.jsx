...
function Content() {
  ...
  const { filteredVideos, selectedCategory } = useFilter();
        ...
        <div className="grid lg:grid-cols-3 sm:grid-cols-2 sm:px-1.5 grid-rows-1 gap-2.5 px-2 perspective-midrange perspective-origin-top">
          {filteredVideos.length > 0
            ...
            : selectedCategory !== "All" && filteredVideos.length === 0 ? (
                // Show message when no videos found for selected category
                ...
                // Show all videos when no category is selected or "All" is selected
                videos.map((video) => {
                  let views = `${video.views.toString().slice(0, -3)}K`;
                  let time = video.uploadDate.split("T")[1].slice(0, -4);
                  return (
